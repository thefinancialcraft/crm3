...
class BackgroundService {
  ...
  static void onStart(ServiceInstance service) async {
    ...
    try {
      ...
      LoggerService.warn('Supabase.initialize in background isolate failed: $e');
    }
    ...
    LoggerService.info('BackgroundService started - notifications will show only during calls');
    
    // Set up real-time call monitoring
    final callSvc = CallLogService();
    await callSvc.initializeBackgroundCallStateListener(flutterLocalNotificationsPlugin);
    ...
    try {
      ...
      if (!isFirstSyncCompleted) {
        LoggerService.info('Performing first sync on app installation (last 3 days only)');
        ...
        LoggerService.info('Enqueued $newCount entries from last 3 days for initial sync');

        if (newCount > 0) {
          // Deduplicate local pending entries against remote Supabase rows for this device
          try {
            final deviceId = await DeviceUtils.getDeviceId();
            LoggerService.info('Deduplicating local entries against Supabase for device $deviceId');

            // Fetch recent remote rows for this device (limit to 3 days window)
            List remoteRows = [];
            try {
              remoteRows = await Supabase.instance.client
                  .from('call_logs')
                  .select('number,timestamp,duration,call_type')
                  .eq('device_id', deviceId)
                  .gte('timestamp', threeDaysAgo.toUtc().toIso8601String()) as List;
            } catch (e) {
            ...
            for (final key in keys) {
              try {
                ...
                if (raw is Map && raw.containsKey('model')) {
                  ...
                  final ts = DateTime.parse(model['timestamp']).toUtc().toIso8601String();
            ...
            LoggerService.info('Deduplication completed, removed $removed duplicate(s)');
          } catch (e) {
